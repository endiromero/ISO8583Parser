...
namespace Fintec.Iso8583
{
	...
	public class ConfigParser
	{
		...
		/// <summary>
		/// Creates a MessageFactory using the default configuration, which is
		/// read from a file specified under the niso8583.config key in the
		/// application's AppSettings.
		/// </summary>
		/// <param name="defaultConfigFileLocation">The default configuration file location.</param>
		public static MessageFactory CreateDefault(string defaultConfigFileLocation)
		{

			if (File.Exists(defaultConfigFileLocation))
			{

				return CreateFromFile(defaultConfigFileLocation);
			}
