	...
	public class FieldParseInfo {
        ...
        public IsoValue Parse(byte[] buf, int pos, Encoding encoder)
        {
			...
			if (type == IsoType.NUMERIC || type == IsoType.ALPHA) {
				...
				return new IsoValue(type, encoder.GetString(buf, pos + 4, length));
			} else if (type == IsoType.LLVARnp) { // added by TAM
                length = ((buf[pos] - 48) * 10) + (buf[pos + 1] - 48);
                if (length < 1 || length > 99)
                {
                    throw new ArgumentException("LLVAR field with invalid length");
                ...
                length = length * 2;
                return new IsoValue(type, encoder.GetString(buf, pos + 2, length));
            } else if (type == IsoType.LLLVARnp) { // added by TAM

                length = ((buf[pos + 1] - 48) * 100) + ((buf[pos + 3] - 48) * 10) + (buf[pos + 5] - 48);
				if (length < 0 || length > 999) {
					throw new ArgumentException("LLLVARnp  field with invalid length");
				}
                length = length * 2;
				return new IsoValue(type, encoder.GetString(buf, pos + 6, length));
